...
function getDailyWordCount() {
  // Get the files
  var folder = DocsList.getFolderById(SANDBOX);
  ...
  for (i in files) {
    total_count += getTotalWordCount(files[i].getId());
  }
  ...
  var word_count = total_count - sheet.getRange("c" + range).getValue();
  ...
  totalCell.setValue(total_count);
  ...
  message = message + "<p>On the " + today + " you wrote " + word_count + " words.</p>";
  if (word_count == 0) {
    message = message + "<p>Wow, you really don't want to be a writer, do you?</p>";
  }
  message = message + "<p>You have written for " + consecutiveCell.getValue() + " days in a row.</p>";
  message = message + "<hr/><p>Your current total word count is: " + total_count + "</p><p>Remember to always do your best!</p></body></html>";
  
  
  // Send the changes to iinet
  Logger.log("  -> Sending email to iinet");
...
// Purpose:  Given a file id, returns the number of words in that file

function getTotalWordCount(id) {
